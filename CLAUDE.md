...
## JunctionArbitration 衝突仲裁系統（設計已確認，實作中）

### 已實作的基礎（Stage A/B/C）
- `BotNormal.cs`：PendingPlannedPath、IsMidSegment()、CanSafelySwapPathNow()、MarkNavigationAnomaly()
- `NavigationAnomalyReason.cs`：異常枚舉
- `LocalTransitionClaimTable.cs`：Stage B（預設關閉）
- `JunctionArbitrationPathManager.cs`：繼承 AgentAStarPathManager，含 4 層仲裁門 + RetryBlockedBots + 死鎖解析
- 驗證 xconf：`Material/Instances/CoreBenchmark/aesop-junction.xconf`

### 下一步：主動衝突偵測與停靠仲裁（尚未實作）

**每 tick Update() 中新增**：
1. `ScanConflicts()` — 掃描所有 non-idle bot 對，Zone(bot) = {CurrentWaypoint ∪ Path前三節點}
2. `AssignStopWaypoints()` — 計算每 bot 最近停靠格，設定 `_holdAt` + `_reservedStopWp`
3. `CheckResolutions()` — 解除已完成 pair，喚醒 bot

**衝突類型與處置**：

| 類型 | 條件 | 高優先者 | 低優先者 |
|------|------|---------|---------|
| RearEnd | 後車 Path 先過前車 CurrentWaypoint | 繼續 | 停在衝突格前一格，喚醒後繼續 |
| HeadOn | 非追撞，有 loaded bot | 繼續 | 停在衝突格前一格，喚醒後繼續 |
| EmptyHeadOn | 非追撞，雙方 Pod==null | 走到衝突格等待 | 停在衝突格前一格，喚醒後重規劃 |

**優先級**：Extract(Tier2) > Loaded非送貨(Tier1) > Empty(Tier0)；同 Tier：垂直 > 水平；Tie：小 ID

**停靠格**：從低優先者自身 Path 推算，`pathList[conflictIdx - 1]`，完全不依賴對方狀態

**Idle robot**（CurrentTask==null/None/Rest）：empty bot A* 重規劃時視為 obstacle（IsLocked=true）

**連鎖衝突**：取最近衝突格停靠（最保守原則）；解除後下一 tick 自動設下一停靠格（方案 B）

### 關鍵 Waypoint 屬性
- `waypoint.PodStorageLocation` — 是否為 pod 儲位
- `waypoint.Paths` — 連接的鄰居（字典，值為距離）
- Loaded bot A*：`CanGoThroughObstacles = false`（不走 pod 儲位）
- Empty bot A*：`CanGoThroughObstacles = CanTunnel`（可穿 pod 儲位）
