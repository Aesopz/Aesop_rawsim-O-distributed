    ...
    public class HADGSManager : OrderManager
    {
        ...
        public double Score()
        {
            ...
            else
            {
                ...
                // PATCH: pod waypoint ID 可能不在 DistanceSet 內（queue wp 等），缺鍵時退化 Manhattan
                var oWp = _currentOStation.Waypoint;
                int oId = oWp.ID;
                return -(40 * completeableAssignedOrders) + CurrentPodtoBot.Sum(v =>
                {
                    double m = Distances.CalculateManhattan1(v.Value.CurrentWaypoint, v.Key.Waypoint);
                    int pId = v.Key.Waypoint.ID;
                    if (DistanceSet.TryGetValue(oId, out var inner) && inner.TryGetValue(pId, out var d))
                        return m + d;
                    return m + Distances.CalculateManhattan1(v.Key.Waypoint, oWp);
                });
            }
