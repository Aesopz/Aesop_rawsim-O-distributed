...
namespace RAWSimO.Core.Control
{
    ...
    public abstract class PathManager : IUpdateable, IStatTracker
    {
        ...
        /// <summary>
        /// Resolves the actual destination waypoint for a bot, accounting for queue positions.
        /// Mirrors the logic in getBotAgentDictionary() lines 347-349.
        /// Required by AgentAStarPathManager because _queueManagers is private.
        /// </summary>
        /// <param name="bot">The bot needing destination resolution.</param>
        /// <param name="destination">The raw destination (bot.DestinationWaypoint).</param>
        /// <returns>The resolved destination — queue position if queued, otherwise destination ?? CurrentWaypoint.</returns>
        internal Waypoint ResolveQueueDestination(BotNormal bot, Waypoint destination)
        {
        ...
        /// <summary>
        /// Returns true if the given waypoint is currently locked by the QueueManager for a
        /// different bot — i.e. physically occupied or reserved by another bot in queue.
        /// Used by AgentAStarPathManager to enforce queue ordering without a full reservation table.
        /// </summary>
        protected bool IsQueueWaypointLocked(Waypoint waypoint, Bot requestingBot)
        {
            foreach (var qm in _queueManagers.Values)
                if (qm.LockedWaypoints.TryGetValue(waypoint, out Bot owner) && owner != requestingBot)
                    return true;
            return false;
        }
        ...
        /// <returns></returns>
        public virtual bool RegisterNextWaypoint(BotNormal botNormal, double currentTime, double blockCurrentWaypointUntil, double rotationDuration, Waypoint waypointStart, Waypoint waypointEnd)
        {
            //get checkpoints
